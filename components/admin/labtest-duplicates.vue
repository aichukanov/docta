...
import type {
	LabTestDuplicateItem,
	LabTestDuplicateSide,
	LabTestDuplicateStats,
} from '~/server/api/labtests/duplicates/queue.get';
...
const items = ref<LabTestDuplicateItem[]>([]);
const stats = ref<LabTestDuplicateStats>({
	pendingA: 0,
	pendingB: 0,
	pendingC: 0,
	dismissed: 0,
...
const TIER_HINTS: Record<'A' | 'B' | 'C', string> = {
	A: 'Совпали названия в двух и более языках либо синоним вместе с языком. Почти всегда настоящий дубликат.',
	B: 'Совпал один язык либо только синоним. Синонимы в базе включают родовые термины и совпадающие аббревиатуры — смотреть внимательно.',
	C: 'Только нечёткое совпадение по английскому. Тир для последней вычитки.',
};

const load = async () => {
	isLoading.value = true;
	try {
		const data = await $fetch('/api/labtests/duplicates/queue', {
...
const sideLabel = (side: LabTestDuplicateSide) =>
	`${side.nameEn || side.nameSr || side.id} (ID: ${side.id})`;

const priceLabel = (side: LabTestDuplicateSide) => {
...
const merge = async (
	item: LabTestDuplicateItem,
	keep: LabTestDuplicateSide,
	drop: LabTestDuplicateSide,
) => {
	const ok = await confirm({
		title: 'Объединить анализы?',
		message:
			`ОСТАВИТЬ: ${sideLabel(keep)}\n` +
			`УДАЛИТЬ: ${sideLabel(drop)}\n\n` +
			'Клиники, цены, категории, справка и синонимы перейдут на оставленный анализ. ' +
			'Названия удаляемого сохранятся как синонимы, со старого адреса встанет 301.',
	...
	await $fetch('/api/labtests/merge', {
		method: 'POST',
		body: {
			primaryLabTestId: keep.id,
			secondaryLabTestId: drop.id,
		},
	});

	// Строка кандидата уходит вместе с удалённым анализом (FK ON DELETE CASCADE),
	// поэтому её отдельно закрывать не нужно — только перечитать очередь.
	await load();
	emit('updated');
};

const dismiss = async (item: LabTestDuplicateItem) => {
	await $fetch('/api/labtests/duplicates/dismiss', {
		method: 'POST',
...
<template>
	<div class="labtest-duplicates">
		<div class="queue-info">
			<p>
				<strong>Очередь дубликатов.</strong> Заполняется скриптом
				<code>scripts/labtests/find-duplicate-labtests.mjs</code>. Отклонённые
			...
			<div v-for="item in items" :key="item.id" class="pair">
				<div class="pair-signals">
					<el-tag
						...
						:key="signal"
						size="small"
						:type="signal === 'synonym-match' ? 'warning' : 'info'"
					>
						{{ signal }}
					</el-tag>
				</div>
				...
				<div class="pair-sides">
					...
					>
						...
						<div class="side-meta">
							...
							<el-tag v-if="side.synonymCount" size="small" type="info">
								синонимов: {{ side.synonymCount }}
							</el-tag>
						</div>
						<el-button
							type="warning"
