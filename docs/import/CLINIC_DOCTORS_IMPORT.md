...
> 🚫 **SQL не запускать самому.** Ассистент только генерирует файл скрипта и выдаёт команду запуска — применяет её пользователь сам. Не выполнять `mysql < ...` через Bash, даже если есть доступ к БД (например, креды из `.env`). Проверочные `SELECT`-запросы для подсчёта существующих врачей до генерации — можно.
