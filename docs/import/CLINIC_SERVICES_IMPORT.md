...
```
medical_services (id, slug, name_en, name_sr, name_sr_cyrl, name_ru, name_de, name_tr, sort_order)
...
```
lab_tests (id, slug, name_en, name_sr, name_sr_cyrl, name_ru, name_de, name_tr)
...
### 1.1 Новые услуги — все 6 языков + slug

> ⚠️ **Поле `slug`** — обязательное (NOT NULL). Генерируется из `name_en` по правилам `common/slug-utils.ts`:
> 1. Транслитерация сербских символов (č→c, ć→c, š→s, ž→z, đ→dj)
> 2. Приведение к нижнему регистру
> 3. Удаление спецсимволов (оставить только `a-z`, `0-9`, пробелы и дефисы)
> 4. Замена пробелов на дефисы
> 5. Удаление дублирующихся дефисов и дефисов по краям
>
> Примеры: `'Cataract Surgery with Vivity IOL'` → `'cataract-surgery-with-vivity-iol'`, `'OCT Angiography'` → `'oct-angiography'`, `'5-Fluorouracil or Mitomycin C Application'` → `'5-fluorouracil-or-mitomycin-c-application'`

```sql
INSERT INTO medical_services (name_en, slug, name_sr, name_sr_cyrl, name_ru, name_de, name_tr) VALUES
('English Name', 'english-name', 'Serbian Latin', 'Српски ћирилица', 'Русский', 'Deutsch', 'Türkçe'),
('Another Service', 'another-service', 'Druga usluga', 'Друга услуга', 'Другая услуга', 'Andere Dienstleistung', 'Başka Hizmet')
...
```sql
-- ✓ Правильно: латинские названия сохранены, slug из name_en
('Chlamydia Trachomatis PCR', 'chlamydia-trachomatis-pcr', 'Chlamydia trachomatis PCR', 'Chlamydia trachomatis PCR', 'Chlamydia trachomatis ПЦР', 'Chlamydia trachomatis PCR', 'Chlamydia trachomatis PCR')

-- ✓ Правильно: немецкий и турецкий переведены
('Histopathology Liver Biopsy', 'histopathology-liver-biopsy', 'PH Biopsija jetre', 'ПХ биопсија јетре', 'Патогистологическое исследование биопсии печени', 'Histopathologie Leberbiopsie', 'Histopatoloji karaciğer biyopsisi')
...
> **⚠️ PROCTOLOGY**: категории PROCTOLOGY нет! Есть только специальность PROCTOLOGY = 36. Для проктологических услуг использовать категорию GENERAL_SURGERY (17) или ABDOMINAL_SURGERY (32).
...
### 2.1 Новые анализы — все 6 языков + slug

> ⚠️ **Поле `slug`** — обязательное (NOT NULL). Генерируется из `name_en` по тем же правилам, что и для услуг (см. раздел 1.1).

```sql
INSERT INTO lab_tests (name_en, slug, name_sr, name_sr_cyrl, name_ru, name_de, name_tr) VALUES
('Complete Blood Count', 'complete-blood-count', 'KKS', 'ККС', 'Общий анализ крови', 'Blutbild', 'Tam Kan Sayımı'),
('C-Reactive Protein', 'c-reactive-protein', 'CRP', 'ЦРП', 'С-реактивный белок', 'CRP', 'CRP')
ON DUPLICATE KEY UPDATE name_en = name_en;
```
...
```sql
-- 1. Скопировать в целевую таблицу (slug нужно сгенерировать вручную!)
INSERT INTO medical_services (name_en, slug, name_sr, name_sr_cyrl, name_ru, name_de, name_tr)
SELECT lt.name_en, lt.slug, lt.name_sr, lt.name_sr_cyrl, lt.name_ru, lt.name_de, lt.name_tr
FROM lab_tests lt
