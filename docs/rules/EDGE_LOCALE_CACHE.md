...
И контрольная — статика в корне с той же cookie обязана пройти насквозь:

```sh
curl -sI -H 'Cookie: locale=de' https://docta.me/favicon.svg | head -1   # 200, не 302
```

Первый прогон на проде здесь и споткнулся: в списке исключений был только
`favicon.ico`, и браузер с сохранённой локалью получал редирект на каждую
иконку. Теперь исключение — любой файл в корне (`ROOT_FILE` в воркере).

Маршрут — именно `docta.me/*`. Вариант `*.docta.me/*` покрывает только
поддомены, апекс под звёздочку не попадает, и воркер молча не вызывается ни
разу — сайт при этом работает как ни в чём не бывало, так что по симптомам это
не отличить от «забыли привязать».
