...
const medicationsListRef = ref<HTMLElement>();
const medicationsMapRef = ref<HTMLElement>();
const PAGE_LIMIT = 50;
...
const medicationsOnPage = computed(() => {
	if (!medicationsList.value?.medications) {
		return [];
	}
	return medicationsList.value.medications.slice(
...
const showClinicOnMap = (clinic: ClinicData) => {
	medicationsMapRef.value.openClinicPopup(clinic);
};
...
const onMapReady = () => {
	watch(
		cityIds,
		() => {
			medicationsMapRef.value?.centerOnLocations(
...
const robotsMeta = computed(() => {
	if (
		...
		medicationsList.value.medications.length === 0
	) {
...
onMounted(async () => {
	...
	watch(pageNumber, () => {
		window.scrollTo(0, 0);
		if (medicationsListRef.value) {
			medicationsListRef.value.scrollTo(0, 0);
		}
	});
});
</script>
...
<template>
	<div class="medications-page">
		<div ref="medicationsListRef" class="medications-sidebar">
			...
			<div class="medications-list-container">
				...
				<div class="medications-list-content">
					<div v-if="isLoadingMedications || isLoadingClinics" class="loading">
					...
					<div v-else class="medications-list">
						<div
							v-if="
								...
								medicationsList.medications.length === 0
							"
							class="empty-state"
						>
						...
						<ListCard
							v-for="medication in medicationsOnPage"
							:key="medication.id"
							:title="medication.name"
							:clinicIds="medication.clinicIds"
							:clinics="clinicsList.clinics"
							:clinicPrices="medication.clinicPrices"
							@show-on-map="showClinicOnMap($event)"
						/>
					</div>

					<Pagination
						v-if="medicationsList?.totalCount"
						:total="medicationsList.totalCount"
		...
		<div class="map-container">
			<DoctorsMap
				...
				:doctors="medicationsList?.medications || []"
				:clinics="clinicsList?.clinics || []"
				@ready="onMapReady"
			/>
		</div>
