...
### Открытый дефект (сверено 2026-07-26)

`page_type` вычисляется внутри колбэка `withMixpanel` в
`composables/use-analytics.ts`, а колбэк откладывается до загрузки SDK
(`mixpanelLoadPromise?.then(...)`). Событие, поставленное в очередь до загрузки,
получит `page_type` той страницы, на которой очередь флашнулась, а не той, где
событие произошло. Задевает в первую очередь события первого экрана. Починка —
считать `page_type` в момент вызова `trackEvent` и передавать в колбэк.
