# Итерация 5: UI компоненты - блок пользователя в хэдере

[← К списку итераций](README.md) | [← Предыдущая](iteration-04-oauth-telegram.md)

---

## Цель

Создать UI для входа/выхода и блок пользователя в хэдере с dropdown меню (для обычных пользователей).

**Примечание:** Страница входа для админов уже создана в Итерации 2.

## Зависимости

Итерации 3-4 (OAuth провайдеры работают)

## Задачи

1. Создать компонент LoginModal с кнопками OAuth
2. Создать компонент UserMenu для хэдера (аватар + dropdown)
3. Создать базовый middleware для защиты страниц (для обычных пользователей)
...
**Назначение:** Модальное окно для входа с выбором OAuth провайдера (для обычных пользователей).
...
**Назначение:** Базовая защита страниц (требует авторизации для обычных пользователей).
...
**Итерация завершена!** Базовая авторизация готова к использованию.

**Следующие шаги:**

- Создать другие PRD (user-profile, doctor-profile, clinic-profile)
- Позже добавить в auth PRD: восстановление пароля для админов, 2FA
