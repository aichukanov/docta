...
**Статус:** 🟢 Complete. Миграции 006/007/008 применены 2026-06-12 (см. `server/sql/migrations/APPLIED.md`) — ожидает только Stripe-ключей  
...
1. ✅ Миграции применены 2026-06-12: `006-clinic-cabinet.sql`, `007-clinic-billing-orders.sql`, `008-billing-remove-dofollow-prices.sql` (последняя вывела DOFOLLOW из продажи — Google link spam policy)
2. ⏳ Завести Stripe test-mode ключи, настроить webhook на `/api/billing/webhook`
3. ⏳ Пройти тестовый цикл с картой `4242 4242 4242 4242`. **Обязательно** прогнать вживую два сценария из code review 2026-07: «две вкладки оплаты» и «отмена → оплата из старой вкладки» — под них специально писались гашение прежних pending-сессий и защита от активации отменённого заказа
4. 💡 v2: автопродление, промокоды, refunds, email об истечении срока

## Известные дефекты (сверено 2026-07-26)

Не блокируют запуск, но чинить лучше до включения боевых ключей:

- `server/utils/billing-email.ts` — `clinicName` подставляется в HTML письма без экранирования (self-injection, low).
- `server/api/billing/webhook.post.ts:234` — дата в письме считается через JS `setMonth`, а в БД пишется `DATE_ADD(NOW(), INTERVAL ? MONTH)`. На конце месяца это разные даты, и в письме окажется не то, что в `valid_until`.
