...
**Запуск:** `node scripts/medicines/build-foreign-products-sql.mjs`. Отчёт печатает
...
- **Код готов, typecheck 0.** Актуальный билд: **2612 продукта, 121 комбо, 2802 связи,
  0 unresolved** (`node scripts/medicines/build-foreign-products-sql.mjs`).
- **ПРОД: переприменить `insert-med-foreign-products.sql`** (последняя регенерация добавила +694
  RU/DE/PL/US-моно для 58 туристических веществ — фикс бага seed). Таблицы `017` на проде уже
  созданы (форма-иконки/матчинг работают в UI). Схема 017 не менялась после — только insert.
  Фикс суппозиториев/формы-по-id — **чисто код, SQL не требует переприменения** (pharma_form_id
  уже в данных). Если сомнение — применить `017` + `insert-med-foreign-products.sql` заново
  (идемпотентно).
