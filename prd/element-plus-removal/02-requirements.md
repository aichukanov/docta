...
| `el-pagination` | 2 | pagination.vue (обёртка), админка | `AppPagination` — номера страниц становятся `<a href>` (EP рендерит `<li class="number">` и разметку наружу не отдаёт), см. FR-10 |
...
- **FR-6.** Существующие обёртки (`pagination.vue`, `filterable-select.vue`, `ApiErrorAlert.vue`, `skeleton-card.vue`) сохраняют свои внешние API — меняется только реализация внутри. Страницы-потребители этих обёрток не трогаются. **Единственное исключение — `pagination.vue`:** её API расширяется необязательным пропом-построителем ссылки (FR-10); потребители, которые его не передают, работают ровно как раньше.
...
- **FR-10. Пагинация краулится.** `AppPagination` рендерит номера страниц (и prev/next) настоящими `<a href>` в SSR-разметке — то, чего нельзя было получить от `el-pagination`, отдающего `<li class="number">` без ссылки. URL берётся у потребителя через проп-построитель, а не собирается внутри компонента: на каталогах это уже существующий `buildUrl` из `paginationLinks` (`components/list-page.vue`), идущий через `getCanonicalUrl`, — иначе разойдётся порядок параметров с canonical. Обычный клик перехватывается и остаётся SPA-переходом (`update:current-page`); клик с модификаторами и средней кнопкой отдаётся браузеру. Где страницы нет в URL (попап карты) — проп не передаётся, рендерятся кнопки. Разбор и таблица потребителей — в [итерации 2](iterations/iteration-02-catalog-filters.md).
