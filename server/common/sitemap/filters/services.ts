...
// Дата изменения фасета — максимум `updated_at` по услугам выборки, поэтому
...
export async function getCategoryIdsWithServices() {
	const lastmod = await lastmodSql('medical_services', 'MAX(ms.updated_at)');
	const connection = await getConnection();

	const query = `
		SELECT mscr.medical_service_category_id as categoryId, ${lastmod} as lastmod
...
export async function getCategoryCityCombinations() {
	...
	const query = `
		SELECT mscr.medical_service_category_id as categoryId, c.city_id as cityId,
			${lastmod} as lastmod
